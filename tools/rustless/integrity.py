...
def create(root,path,include=(),exclude=()):
    ...
    data=manifest(root,include,exclude+(str(path),)); p.parent.mkdir(parents=True,exist_ok=True); p.write_text(json.dumps(data,indent=2,sort_keys=True)+'\n'); return data
def verify(root,path):
    m=json.loads((root/path).read_text()); expected={x['path']:x for x in m.get('files',[])}; actual={r:{'path':r,'size':p.stat().st_size,'sha256':sha256_file(p)} for p,r in iter_files(root,exclude=(str(path),))}
    changes=[]
